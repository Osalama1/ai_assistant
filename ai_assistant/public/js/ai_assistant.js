...
class AIAssistant {
    ...
    async sendQuery(query, options = {}) {
        ...
        try {
            ...
            if (response.success) {
                ...
                this.addChatMessage(`Sorry, I encountered an error: ${response.error}`, 'ai');
            }
    ...
    async sendQuickQuery(query) {
        const response = await fetch('/api/method/ai_assistant.api.chat.quick_query', {
            method: 'POST',
            headers: {
    ...
    async sendComplexQuery(query, queryType) {
        const response = await fetch('/api/method/ai_assistant.api.chat.chat_with_ai', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Frappe-CSRF-Token': this.getCSRFToken()
        ...
        if (data.message && data.message.query_id) {
            // Poll for response
            return await this.pollForResponse(data.message.query_id);
        }
        
        return data.message || data;
    }

    async pollForResponse(queryId, maxAttempts = 10) {
        for (let i = 0; i < maxAttempts; i++) {
            await this.sleep(2000); // Wait 2 seconds

            const response = await fetch('/api/method/ai_assistant.api.chat.get_chat_response', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Frappe-CSRF-Token': this.getCSRFToken()
    ...
    async uploadFile(file) {
        try {
            ...
            if (uploadData.message) {
                // Process document
                const processResponse = await fetch('/api/method/ai_assistant.api.document_analysis.upload_document', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-Frappe-CSRF-Token': this.getCSRFToken()
    ...
    async monitorProcessing(processorId, fileName) {
        ...
        const checkStatus = async () => {
            try {
                const response = await fetch('/api/method/ai_assistant.api.document_analysis.get_processing_status', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-Frappe-CSRF-Token': this.getCSRFToken()
